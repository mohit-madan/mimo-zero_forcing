...
int main()
{
  ...
  // matrix
  int nrof_samples = Number_of_bits*7/8;
  cmat ch_coeffs;
  ...
  //for (i = 0; i < 1; i++) {
    ...
    transmitted_symbols = elem_mult(transmitted_symbols, fade_c);
	...
	received_symbols = elem_div(received_symbols, fade_c);    
    // for(int j=0;j < nrof_samples;j++){
...
//    cout << received_bits;
  ...
  cout << "Saving results to ./qam4ham_result_file.it" << endl;

  cout << endl;
  //Save the results to file:
  ff.open("qam4ham_result_file.it"); 
